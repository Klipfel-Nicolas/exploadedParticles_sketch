...
void main() {
  ...
  vec3 distortionFinal = vec3(position.x * 2., position.y, 1.) * curl_noise(vec3(
    position.x * .002 + time * 0.1, 
    position.y * .008 + time * 0.1, 
    (position.x + position.y) * 0.02
  )) * distortion;

  vec3 finalPosition = position + distortionFinal;
